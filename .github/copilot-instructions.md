...
**Before editing backend code (`backend/**`), read** [`.github/instructions/backend.instructions.md`](.github/instructions/backend.instructions.md) **for language-specific conventions.**
...
- Solution file: `backend/WroclawTheatreTickets.slnx`
...
- If no source projects are present, prompt the human owner for the missing `backend/` or solution content.
...
- Build the solution: `dotnet build backend/WroclawTheatreTickets.slnx`.
...
- The single discoverable artifact is the solution file; don’t assume folder layout. Always search for `backend/src/`, `backend/tests/`, and `docs/`.
...
- Add or update unit tests in a `backend/tests/` project when behaviour is changed.
